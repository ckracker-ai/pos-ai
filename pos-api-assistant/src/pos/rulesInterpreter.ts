import type { PosAiResult, PosCartItem, PosInterpretInput, PosStockItem } from './types.js';
...
function findProductByText(text: string, stocks: PosStockItem[]): PosStockItem | null {
  ...
  let best: { p: PosStockItem; score: number } | null = null;
...
function resolveProduct(
  ...
  if (preferCart) {
    const inCart = findProductByText(text, stocksInCart(stocks, cart));
    if (inCart) return inCart;
  }
  return findProductByText(text, stocks);
}
...
export function interpretPosCartRules(input: PosInterpretInput): PosAiResult {
  ...
  const direct = findProductByText(text, input.stocks);
