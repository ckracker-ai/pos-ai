...
export async function seedBootstrapAdmin(): Promise<void> {
  ...
  // Sequelize advierte sobre class fields públicos en modelos; usando getDataValue
  // evitamos leer propiedades "shadowed" como undefined.
  const roleId = String(role.getDataValue('id') ?? '');
  const branchId = String(branch.getDataValue('id') ?? '');

  if (!roleId || !branchId) {
    console.warn('⚠️  Bootstrap admin skipped: roleId/branchId unresolved');
    return;
  }
