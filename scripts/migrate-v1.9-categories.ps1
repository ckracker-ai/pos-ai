...
Get-ChildItem "$root\db-init\migrations\v1.9.0\*.sql" | Sort-Object Name | ForEach-Object {
  Write-Host "Applying $($_.Name)..."
  Get-Content $_.FullName -Raw | docker exec -i pos-ai-db-mysql mysql -uroot "-p$password" pos-ai-db
}

Write-Host "OK. Reinicia servicios: docker compose up -d --build pos-api-core pos-api-bff pos-frontend"
