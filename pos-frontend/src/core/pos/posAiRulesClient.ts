...
import type { PosAiProductOption, PosAiResult } from './posAiTypes';
import { formatPosHelpMessage } from './posSaleAssist';
...
type ScoredProduct = { p: PosAiStockItem; score: number };

function scoreProduct(text: string, p: PosAiStockItem): ScoredProduct | null {
...
function rankProducts(text: string, stocks: PosAiStockItem[]): ScoredProduct[] {
  return stocks
...
function pickAmongSameVariant(text: string, items: PosAiStockItem[]): PosAiStockItem {
  const tokens = meaningfulTokens(text);
...
function filterAmbiguousOptions(text: string, options: PosAiStockItem[]): PosAiStockItem[] {
  const variants = variantTokensInText(text);
...
type ProductPickResult =
  | { ok: true; product: PosAiStockItem }
  | { ok: false; reason: 'none' }
  | { ok: false; reason: 'ambiguous'; options: PosAiStockItem[] };

function pickByVariantTokens(text: string, ranked: ScoredProduct[]): PosAiStockItem | null {
  const variants = variantTokensInText(text);
...
function pickBySizePriceHint(text: string, ranked: ScoredProduct[]): PosAiStockItem | null {
  const tokens = meaningfulTokens(text);
  const wantsLarge = tokens.some((t) => SIZE_LARGE_HINTS.has(t));
  const wantsSmall = tokens.some((t) => SIZE_SMALL_HINTS.has(t));
...
function pickProduct(text: string, stocks: PosAiStockItem[]): ProductPickResult {
...
function formatProductOption(p: PosAiStockItem): string {
  const cat = p.categoria?.trim();
  return cat ? `${p.nombre} (${cat})` : p.nombre;
}

function toProductOptions(items: PosAiStockItem[], limit = 8): PosAiProductOption[] {
...
function searchListMessage(options: PosAiStockItem[], query: string): string {
  if (options.length === 0) {
