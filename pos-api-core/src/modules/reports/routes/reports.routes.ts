...
router.get('/summary', requireSeller, async (req: AuthenticatedRequest, res) => {
  ...
  const result = await reportsDelegate.getSummary(empresaId, branchId);
...
router.get('/revenue-trend', requireSeller, async (req: AuthenticatedRequest, res) => {
  ...
  const result = await reportsDelegate.getRevenueTrend(empresaId, branchId, days);
...
router.get('/low-stock', requireSeller, async (req: AuthenticatedRequest, res) => {
  ...
  const result = await reportsDelegate.getLowStockAlerts(empresaId, branchId, limit);
...
router.get('/sales', requireSeller, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const result = await reportsDelegate.getSalesTable(empresaId, branchId, limit);
...
router.get('/inventory', requireSeller, async (req: AuthenticatedRequest, res) => {
  try {
    const empresaId = getEffectiveEmpresaId(req);
    const branchId = resolveReportBranchId(req);
    const result = await reportsDelegate.getInventoryTable(empresaId, branchId);
...
router.get('/shrinkage', requireSeller, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const result = await reportsDelegate.getShrinkageReport(empresaId, branchId, { status, limit });
    if (result.success) return sendOk(res, result.value);
    return sendFail(res, result.error, 500);
  } catch {
...
router.get('/hot-skus', requireSeller, async (req: AuthenticatedRequest, res) => {
  const empresaId = getEffectiveEmpresaId(req);
  const branchId = getEffectiveBranchId(req);
  const hourRaw = req.query.hour != null ? Number(req.query.hour) : undefined;
  const result = await reportsDelegate.getHotSkus(empresaId, branchId, hourRaw);
  if (result.success) return sendOk(res, result.value);
  return sendFail(res, result.error, 500);
});

router.get('/reorder-draft', requireSeller, async (req: AuthenticatedRequest, res) => {
  const empresaId = getEffectiveEmpresaId(req);
  const branchId = resolveReportBranchId(req);
  const limit = Math.min(50, Math.max(5, Number(req.query.limit ?? 20)));
  const result = await reportsDelegate.getReorderDraft(empresaId, branchId, limit);
  if (result.success) return sendOk(res, { drafts: result.value, status: 'DRAFT' });
  return sendFail(res, result.error, 500);
});
...
router.get('/dashboard', requireSeller, async (req: AuthenticatedRequest, res) => {
  ...
  const [summary, trend, alerts, hot, reorder, biz] = await Promise.all([
    reportsDelegate.getSummary(empresaId, branchId),
    reportsDelegate.getRevenueTrend(empresaId, branchId, days),
    reportsDelegate.getLowStockAlerts(empresaId, branchId, 20),
    reportsDelegate.getHotSkus(empresaId, getEffectiveBranchId(req)),
    reportsDelegate.getReorderDraft(empresaId, branchId, 20),
    reportsDelegate.getBusinessInsights(empresaId, branchId),
  ]);
  ...
  const insights = biz.success
    ? {
        ...biz.value,
        todaySales: summary.value.todaySales,
        todayRevenue: summary.value.todayRevenue,
      }
    : null;
  ...
  return sendOk(res, {
    ...
    days,
    hotSkus: hot.success ? hot.value : { hour: 0, items: [] },
    reorderDrafts: reorder.success ? reorder.value : [],
